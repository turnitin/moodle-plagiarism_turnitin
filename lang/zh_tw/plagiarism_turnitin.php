...
/**
 * Strings for component 'qtype_kprime', language 'zh_tw'
 *
...
$string['anonblindmarkingnote'] = "注意事項：個別的 Turnitin 匿名標記設定已經移除。Turnitin 將使用 Moodle 的 [Anonymous submissions]（匿名提交）設定來決定匿名標記設定。";
...
$string['reportgenspeed_help'] = "此作業設定有三個選項：「立即產生報告。提交將立即新增至存放庫 (如果已經設定存放庫)。」、「立即產生報告。提交將在截止日期新增至存放庫 (如果已經設定存放庫)。」，以及「在截止日期產生報告。提交將在截止日期新增至存放庫 (如果已經設定存放庫)。」<br /><br />「立即產生報告。提交將立即新增至存放庫 (如果已經設定存放庫)。」選項可在學生提交時立即產生「原創性報告」，對於遲交則會和先前的提交進行比較 (如果存放庫儲存有先前的提交的話)。<br /><br />「立即產生報告。提交將在截止日期新增至存放庫 (如果已經設定存放庫)。」選項可在學生進行提交時立即產生「原創性報告」，並會在作業的截止日期執行「合謀檢查」(如果存放庫儲存有先前的提交的話)。<br /><br />「在截止日期產生報告。提交將在截止日期新增至存放庫 (如果已經設定存放庫)。」選項可產生「原創性報告」，並在作業的截止日期執行「合謀檢查」(如果存放庫儲存有先前的提交的話)。";
...
$string['enableadhocsubmissions'] = "啟用臨時工作，處理提交佇列";
$string['adhoc_sendqueuedsubmission'] = "臨時工作：從 Turnitin Plagiarism Plugin 傳送排入佇列的提交";
