...
$string['reportgenspeed_help'] = "この課題の設定には、次の3つのオプションがあります。直ちにレポートを生成する：答案はすぐにリポジトリに追加されます（リポジトリが設定されている場合）。 直ちにレポートを生成する：答案は提出期限にリポジトリに追加されます（リポジトリが設定されている場合）。 提出期限にレポートを生成する：答案は提出期限にリポジトリに追加されます（リポジトリが設定されている場合）。<br /><br />「直ちにレポートを生成する」オプションでは、答案がすぐにリポジトリに追加されます（リポジトリが設定されている場合）。また、学生が提出を行うとすぐにOriginalityレポートが生成され、リポジトリに保存されている以前の答案と新しい答案が比較されます。<br /><br />直ちにレポートを生成する：答案は提出期限にリポジトリに追加されます（リポジトリが設定されている場合）。また、学生が提出を行うとすぐにOriginalityレポートが生成され、答案がリポジトリに保存されている場合は、課題の提出期限に結託を見つけるためのチェックが実行されます。<br /><br />提出期限にレポートを生成する」オプションでは、答案が提出期限にリポジトリに追加されます（リポジトリが設定されている場合）。また、答案がリポジトリに保存されている場合は、課題の提出期限にOriginalityレポートが生成され、結託を見つけるためのチェックが実行されます。";
$string['turnitinuseanon_desc'] = '提出物評価時に匿名コメントの記入（マーキング）を許可するかどうかを設定してください。<br /><i>（このオプションは、アカウントで匿名コメントの記入を設定している方にのみ利用可能です）</i>';
$string['nonsubmittingstudentinfo'] = "為了檢視此群組提交的 Turnitin 報告或意見回饋，請聯絡代理群組提交的指導講師或學生。";

$string['enableadhocsubmissions'] = "提出キューのアドホックタスク処理を有効にする";
$string['adhoc_sendqueuedsubmission'] = "アドホックタスク：Turnitinの剽窃プラグインからキューに入っている提出物を送信する";
